...
// Fetch admin's ID from the session (assuming the admin is logged in)
$adminId = $_SESSION['a_id']; // Admin's ID should be stored in session
...
$sql = "SELECT lr.leave_id, e.e_id, e.firstname, e.lastname, e.department, e.available_leaves, lr.start_date, lr.end_date, lr.leave_type, lr.proof, lr.status
        ...
        WHERE lr.supervisor_approval = 'Supervisor Approved' AND lr.status = 'Supervisor Approved'";
    ...
    if ($action_result->num_rows > 0) {
        ...
        if ($status === 'approve') {
            ...
            if ($leave_days > $available_leaves) {
                ...
                // Update leave request status to 'Admin Approved' and subtract days from available leave balance
            ...
            // Deny the leave request and record the admin ID
...
<div class="modal fade" id="confirmActionModal" tabindex="-1" aria-labelledby="confirmActionModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content bg-dark">
            <div class="modal-header">
                <h5 class="modal-title text-light" id="confirmActionModalLabel">Confirm Action</h5>
                <button type="button" class="close text-light bg-dark" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span> <!-- This is the X button -->
                </button>
            </div>
            <div class="modal-body text-light">
                <p  id="modalMessage"><center>Are you sure you want to proceed with this action?</p></center>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    let calendar;
        ...
        //LEAVE STATUS 
        // Function to show the modal and handle action
        function confirmAction(action, requestId) {
    const modal = new bootstrap.Modal(document.getElementById('confirmActionModal'));
    const modalMessage = document.getElementById("modalMessage");
    const confirmBtn = document.getElementById("confirmBtn");
    const cancelBtn = document.getElementById("cancelBtn"); // Cancel button reference
    const closeButton = document.querySelector("#confirmActionModal .close"); // X button reference

    // Set the message dynamically based on the action
    //modalMessage.textContent = `Are you sure you want to ${action} this leave request?`;
    ...
    // If the "X" button (close button) is clicked, close the modal
